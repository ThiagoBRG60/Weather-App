...
let isToastCardHidden = false
let toastCardStack = []
let isToastStackActive = false

toastCardCloseButton.addEventListener("click", () => isToastCardHidden = true)
      ...
      for (let i = 0; i < toastCardStack.length; i++) {
         ...
         setTimeout(() => {
            const intervaldId = setInterval(() => {
               if (toastCardSeconds.toFixed(2) <= 0 || isToastCardHidden) {
                  setTimeout(() => toastCard.className = `toast-card ${toastCardStack[i].status}`, isToastCardHidden ? 0 : 500)
                  setTimeout(() => {
                     toastCardTimerBar.style.width = "100%"
                     toastCardSeconds = 3
                     if (isToastCardHidden) sleepFn.cancel()
                  }, 1000)
